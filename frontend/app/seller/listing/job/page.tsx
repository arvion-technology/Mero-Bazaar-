...
export default function NewJobListingPage() {
  ...
  const handleSalaryMinChange = (val: string) => {
    const cleaned = val.replace(/[^0-9]/g, "");
    setSalaryMin(cleaned);
  };

  const handleSalaryMaxChange = (val: string) => {
    const cleaned = val.replace(/[^0-9]/g, "");
    setSalaryMax(cleaned);
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!salaryMin || !salaryMax) {
      toast.error("Please fill salary range");
      return;
    }
  ...
  const formattedSalaryMin = useMemo(() => {
    if (!salaryMin) return "";
    const num = Number(salaryMin.replace(/,/g, ""));
    if (isNaN(num)) return salaryMin;
    return num.toLocaleString("en-IN");
  }, [salaryMin]);

  const formattedSalaryMax = useMemo(() => {
    if (!salaryMax) return "";
    const num = Number(salaryMax.replace(/,/g, ""));
          ...
          <form onSubmit={handleSubmit} className="form-card">
            ...
            <div className="options-row">
              <label className="checkbox-label">
                ...
                />
                Urgent Hiring (show urgent badge on listing)
              </label>
              <label className="checkbox-label">
                <input
                  type="checkbox"
                ...
                />
                Employer Phone Verified (show verified badge)
              </label>
            </div>

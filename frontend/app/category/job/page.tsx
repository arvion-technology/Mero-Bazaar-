...
import { FiSearch, FiMapPin, FiHeart, FiPhone, FiMessageSquare, FiChevronDown, FiBookmark, FiTarget } from "react-icons/fi";
...
const JOBS: Job[] = [
  {
    ...
    company: "XYZ Corporation Pvt LTD",
  ...
  },
  {
    ...
    company: "Pizza House",
    salary: "NPR 1,500–2,000/month",
    location: "Kathmandu, Bagmati",
    district: "Kathmandu",
    type: "Full Time",
    thumb: "/job3.jpg",
    skills: ["Riding", "Delivery"],
    category: "Gig/Freelance",
    minSalary: 1500,
    postedDaysAgo: 3,
  },
  {
    ...
    type: "Full Time",
    thumb: "/job4.jpg",
    skills: ["Design", "Photoshop", "Negotiation"],
    category: "Design",
    minSalary: 30000,
    postedDaysAgo: 2,
  },
...
const JOB_TYPES = ["Full-time", "Part-time", "Gig/Freelance", "Construction"];
const CITIES = ["Kathmandu", "Lalitpur", "Bhaktapur", "Pokhara", "Chitwan", "Biratnagar", "Butwal"];
const SKILLS = ["Sales", "Accounting", "Design", "Marketing", "IT & Software", "Construction", "Riding"];
...
export default function JobsPage() {
  ...
  const displayed = JOBS.filter((j) => {
    ...
    if (s && !j.title.toLowerCase().includes(s) && !j.company.toLowerCase().includes(s) && !j.location.toLowerCase().includes(s)) return false;
    if (city && !j.district.toLowerCase().includes(city.toLowerCase())) return false;
    if (skill && !j.skills.some(sk => sk.toLowerCase().includes(skill.toLowerCase())) && !j.category.toLowerCase().includes(skill.toLowerCase())) return false;
    if (minSalary && j.minSalary < parseInt(minSalary)) return false;
    if (selectedTypes.length) {
      const normalize = (t: string) => t.toLowerCase().replace(/[^a-z]/g, "");
      const match = selectedTypes.some(t => normalize(j.type).includes(normalize(t)) || normalize(j.category).includes(normalize(t)));
    ...
    <>
      <style>{`
        ...
        .jp-hero-bg {
          ...
          background: url('/job1.jpg') center / cover no-repeat;
          filter: brightness(0.52);
        }
        .jp-hero-overlay {
          position: absolute; inset: 0;
          background: linear-gradient(135deg, rgba(10,30,90,0.75) 0%, rgba(5,15,50,0.5) 100%);
        }
        .jp-hero-wm {
        ...
        .jp-search {
          ...
          background: rgba(255,255,255,0.97); border: none; border-radius: 12px;
        ...
        /* ─── SIDEBAR ─── */
        .jp-sidebar {
          width: 220px; flex-shrink: 0; background: #fff; border-radius: 16px;
        ...
        .jsb-select {
          ...
          background: #fafafa; appearance: none; outline: none; cursor: pointer;
        ...
        .jsb-salary-unit {
          font-size: 12px; color: #999; font-weight: 500; white-space: nowrap;
        }
        ...
        .jsb-apply {
          ...
          box-shadow: 0 4px 14px rgba(192,57,43,0.3); transition: opacity 0.18s;
        }
        ...
        @media (max-width: 600px) {
          .jp-body { padding: 16px 14px 60px; }
          .jp-hero { height: 220px; }
          .jp-card-body-row { gap: 12px; }
          .jp-thumb { width: 64px; height: 64px; }
          .jp-card-title { font-size: 16px; }
          .jp-btn { padding: 9px 8px; font-size: 12.5px; }
        }
      `}</style>
                ...
                <div className="jp-list">
                  {displayed.map((j) => {
                    ...
                    const typeClass =
                      ...
                      : j.type.toLowerCase().includes("construction") ? "construction"
                      : "";

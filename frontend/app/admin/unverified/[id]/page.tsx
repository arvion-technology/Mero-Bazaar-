...
const sidebarItems = [
  ...
  { id: "unverified", icon: FiUser, label: "Unverified List", active: true },
];

export default function UnverifiedKYCDetailPage() {
    ...
    return (
      <>
        <style>{`
          ...
          @media (max-width: 1023px) { .admin-sidebar { transform: translateX(-100%); transition: transform 0.3s ease; width: 80% !important; max-width: 300px; z-index: 200; box-shadow: none; } .admin-sidebar.mobile-open { transform: translateX(0); box-shadow: 4px 0 32px rgba(0,0,0,0.15); } .admin-sidebar.mobile-open .admin-sidebar-close { display: flex; } .admin-hamburger { display: flex; } .admin-main { margin-left: 0 !important; max-width: 100% !important; } }
        `}</style>
        <div className="admin-page">
          <aside className={`admin-sidebar ${sidebarOpen ? "mobile-open" : ""}`}>
            <button type="button" className="admin-sidebar-close" onClick={() => setSidebarOpen(false)}><FiX size={18} /></button>
              ...
              {sidebarItems.map((item) => (
                item.href ? (
                  <Link key={item.id} href={item.href} className={`admin-nav-item ${activeTab === item.id ? "active" : ""}`} onClick={() => handleNavClick(item.id)}>
                    <span className="admin-nav-icon"><item.icon size={18} /></span><span>{item.label}</span>
                  </Link>
                ) : (
    ...
    <>
      ...
      <div className="admin-page">
        ...
        <main className="admin-main">
          <div className="admin-topbar">
            <div className="admin-topbar-left">
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                <Link href="/admin/unverified" className="admin-back-link">
          ...
          <KYCDetailsContentPending kyc={kyc} />
        </main>
      </div>
    </>
  );
